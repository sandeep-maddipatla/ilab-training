...
run() {
    ...
    if [[ "$cmd" == *"PT_TOWL_LOG_ENABLE=1"* ]]; then
        # Export additional environment variables as requested
        ...
        export PT_TOWL_LOG_CONFIG="rank=any"
        export LOG_FILE_SIZE=1048576000
        export PT_LOG_FILE_AMOUNT=100
        echo "PT_TOWL_LOG_ENABLE=1 detected, setting additional environment variables for TOWL logging"
        env > ${result_dir}/env.log
    fi
    ...
    mv *.hltv ${result_dir}
    mv metricslog-rank*.json ${result_dir} 2>/dev/null
