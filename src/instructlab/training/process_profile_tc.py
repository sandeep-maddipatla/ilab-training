...
def process_tpc_slices_fwd(tpc_slices, start_ts=0, end_ts=float('inf')):
    ...
    layer_start_ts = None
    layer_end_ts = None
    rope_st2_fwd_bf16_count = 0
    prev_block_end_ts = None

    for s in tpc_slices:
        if s.ts < start_ts:
            continue
        
        if s.name and 'rope_st2_fwd_bf16' in s.name:
            if rope_st2_fwd_bf16_count == 6:
                # hit the first block of the next layer
                layer_end_ts = prev_block_end_ts
            ...
            rope_st2_fwd_bf16_count += 1

        elif s.name and 'logsoftmax_fwd_f32' in s.name and layer_start_ts:
...
def process_tpc_slices_bwd(tpc_slices, start_ts=0, end_ts=float('inf')):
    ...
    for s in tpc_slices:
        ...
        if s.name and 'rms_norm_bwd_bf16' in s.name:
            if rms_norm_bwd_bf16_count == 4:
                # hit the first block of the next layer
                layer_end_ts = prev_block_end_ts
                layer_ts_list.append({ 'start': layer_start_ts, 'end': layer_end_ts, 'duration': layer_end_ts - layer_start_ts , 'bubble': 0 if not layer_ts_list else layer_start_ts - layer_ts_list[-1]['end']})
            ...
            # we will approximate that it will end at this
            if layer_start_ts:
...
def print_summary(layers, tag=""):
    ...
    if layer_durations:
        print(f'{tag}: Duration Time (ms): min={min(layer_durations):.3f}, max={max(layer_durations):.3f}, avg={statistics.mean(layer_durations):.3f}, median={statistics.median(layer_durations):.3f} .. {len(layer_durations)} entries')
    else:
        print(f'{tag}: Duration Time (ms): No entries')
    if bubble_times:
        print(f'{tag}: Bubble Time (ms): min={min(bubble_times):.3f}, max={max(bubble_times):.3f}, avg={statistics.mean(bubble_times):.3f}, median={statistics.median(bubble_times):.3f} .. {len(bubble_times)} entries')
    else:
        print(f'{tag}: Bubble Time (ms): No entries')
